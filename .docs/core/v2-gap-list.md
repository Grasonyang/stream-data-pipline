...
| ~~`pipeline_dispatcher` child exec path / failure propagation 仍可更明確~~ **Done (v2.1)**: spawn 失敗時呼叫 `kill_and_reap()` 清理已啟動的 child；`wait_all()` 加入 pid < 0 guard 避免 `waitpid(-1,...)`；failure behavior 記錄至 `.docs/applets/pipeline-dispatcher.md` | final demo 需要能說明 fork/exec/waitpid、exit code 與 failure behavior | GRA-22 | Yes |
