...
- `stream_merge` now follows the session-level `.bin` buffer plus `.meta.jsonl` sidecar model; remaining work is limited to future CRC/events/physical extraction scope rather than the old JSON-in-bin mismatch.
