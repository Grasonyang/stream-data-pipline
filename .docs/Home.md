...
2. 再看 `core/compliance-matrix`，確認作業要求、目前狀態與後續 milestone。
3. 接著看 `core/full-spec`，了解本 repo 的內部邊界、資料流、process topology 與 stdout/stderr 規則。
4. 要排 v2.1/v2.2 工作時，看 `core/v2-gap-list`。
5. 要改 orchestration 或 pipeline 行為時，先讀 `applets/pipeline-dispatcher`。
6. 要改某個 applet 時，讀對應 applet spec，再看 `lib/libpipeline` / `lib/stream-logger` 是否已定義共用行為。
7. 跨 repo 行為不要在這裡新增完整規格，請更新 Linear。

## Suggested Reading By Task

- 想快速跑起來：`README.md` -> `core/full-spec` -> `applets/pipeline-dispatcher`
- 想確認課程要求是否對齊：`README.md` -> `core/compliance-matrix` -> `core/full-spec`
- 想規劃 v2.1/v2.2：`core/compliance-matrix` -> `core/v2-gap-list` -> Linear milestone issues
- 想修改 `stream_merge` / `log_parse` / `clip_store`：`core/full-spec` -> applet spec -> `lib/libpipeline` / `lib/stream-logger`
- 想追 integration contract：直接看 Linear，不要只依賴 repo docs

## Pages

- `core/full-spec`：本 repo 的內部總覽，先定義 role、boundary、data flow 與 topology。
- `core/compliance-matrix`：作業 B + 方向三要求對照表，含完成狀態與 follow-up routing。
- `core/v2-gap-list`：v2.1/v2.2 缺口列表與 Linear issue planning。
- `applets/pipeline-dispatcher`：entry point、fork/exec、pipe topology、exit code。
- `applets/stream-merge`：讀取 `.meta.jsonl` sidecar、監聽 sentinel、輸出 clip byte-range metadata JSON Lines。
- `applets/log-parse`：regex-based 結構化日誌解析、JSON/CSV 輸出、integration filter。
- `applets/clip-store`：純文字 index、TTL、GC 與查詢行為。
- `lib/libpipeline`：共用低階 helper，例如 inotify watch 與 sentinel helper。
- `lib/stream-logger`：stderr-only 診斷輸出與 log API。
